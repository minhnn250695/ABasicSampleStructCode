...
declare var $: any;
@Component({
    selector: 'app-macquarie-unmatch',
    templateUrl: './macquarie-unmatch-records.component.html',
    styleUrls: ['./macquarie-unmatch-records.component.css']
})
export class MacquarieUnMatchComponent extends DataFeedsBaseComponent implements OnInit {
            ...
            this.dataFeedsService.updateClientAssetRecord(this.selectedEntity, null, true)
                .subscribe(responseCode => {
                    if (responseCode === 200) {
                        ...
                        $('#macquarie-overwrite-warning').modal({
                            backdrop: 'static',
                        });
                        $("#macquarie-overwrite-warning").modal();
